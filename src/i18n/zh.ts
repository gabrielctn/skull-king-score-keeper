...
export const zh: Strings = {
  ...
  home: {
    ...
    activeTitle: "进行中的对局",
    playing: "参与玩家",
    lastPlayed: (date) => `上次活动：${date}`,
    abandon: "放弃对局",
    abandonTitle: "放弃这局对局？",
    abandonMessage:
      "这局进行中对局的全部分数和回合详情将被永久删除。已结束的对局不会受到影响。",
    abandonConfirm: "放弃",
    history: "最近对局",
    historyHint: "点击一局对局以继续或查看排名。",
    historyShowAll: (count) => `显示全部 ${count} 局`,
    historyShowLess: "收起",
    inProgress: "进行中",
    finished: "已结束",
    openGame: (date) => `打开 ${date} 的对局`,
    deleteGame: (date) => `删除 ${date} 的对局`,
    deleteTitle: "删除这局对局？",
  ...
  whatsNew: {
    ...
    items: [
      "地图背景现在会铺满整个屏幕，不再只是角落里的一个小方块；在手机上缩小页面时，页面也不会再向两侧滑动。",
      ...
      "邀请伙伴和加入另一张牌桌现在是两个独立操作，可直接在设置中看到。",
      "共享牌桌：给你们的牌桌取个名字，通过链接或二维码邀请朋友。加入的人都可以计分，每场对局都会记入共享历史和排行榜；一部手机还能保存多张牌桌，每群朋友一张。",
      "现在可以在对局中通过新的 ⚙ 按钮修改规则：中途启用扩展、切换计分方式等。已计分的回合会自动重新计算。",
      "新对局现在默认启用新扩展。",
      "实时跟踪按钮改为清晰的“Live”标签，出牌顺序显示编号座位并标明首出玩家。",
      "所有奖励名称现在采用统一的表述：谁俘获了什么。",
    ],
    close: "知道了",
  },
  ...
  settings: {
    ...
    keepAwakeHint: "对局界面打开时，阻止设备进入休眠。",
    dataTitle: "你的数据",
    dataHint: "导出对局备份，或在此设备上恢复备份。",
    exportBackup: "导出",
    importBackup: "导入",
    importSuccess: (count) => `已导入 ${count} 局对局。`,
    backupError: "无法读取此备份。",
    deleteAll: "删除全部对局",
    deleteAllTitle: "删除全部对局？",
    deleteAllMessage: "所有对局（包括进行中的一局）都将被永久删除。建议先导出备份。",
    deleteAllCancel: "取消",
    deleteAllConfirm: "全部删除",
    deleteAllSuccess: "已删除全部对局。",
    feedbackTitle: "反馈",
    ...
    cloud: {
      ...
      tableUnnamed: "未命名牌桌",
      tableActive: "当前",
      tableSwitch: (name) => `切换到牌桌“${name}”`,
      tableSwitching: "正在打开该牌桌…",
      tableSwitchError: "无法打开该牌桌。请检查网络连接后重试。",
      newTable: "新建牌桌",
      newTableHint:
        "一张全新的牌桌，拥有自己的历史和排行榜，适合另一群朋友。你现有的牌桌会保留在本手机上。",
      removeTable: (name) => `从本手机移除“${name}”`,
      removeTableTitle: "从本手机移除这张牌桌？",
      removeTableMessage:
        "它只会从本手机消失。牌桌及其对局对其他成员仍然存在，之后你可以通过邀请重新加入。",
      removeTableCancel: "取消",
      removeTableConfirm: "移除",
      tableNameLabel: "牌桌名称",
      tableNamePlaceholder: "例如：周五之夜船队",
  ...
  joinTable: {
    title: "加入牌桌",
    named: (name) => `加入牌桌“${name}”？`,
    unnamed: "加入这张牌桌？",
    message: (count) =>
      `这张牌桌及其 ${count} 场对局将添加到本手机。本手机已有的对局会留在各自的牌桌中；你可以随时切换牌桌。`,
    confirm: "加入牌桌",
    cancel: "暂不加入",
    busy: "正在加入…",
    success: "欢迎登船！本手机现在关注这张牌桌。",
    error: "无法打开此邀请。请检查网络连接，或请伙伴重新发送链接。",
  },
  ...
  setup: {
    ...
    scoringHints: {
      ...
      rascal:
        "每回合的赌注为每发一张牌 10 分。叫牌准确：全部获得。相差 1：获得一半。相差 2 或以上：一无所获，永远不会扣分。",
    },
  ...
  game: {
    ...
    cardsDealt: "张牌",
    dealsVerb: "发牌",
    playOrderLead: (name) => `出牌顺序 · ${name} 首出第一墩`,
    ghostName: "灰胡子",
    bid: "叫牌",
    won: "赢得墩数",
    bonus: "奖励",
    roundPoints: "本回合得分",
    roundPointsPreview: "暂计得分",
    total: (n) => `总分 ${n}`,
    ...
    rascalBetFor: (name) => `${name} 的选择`,
  },

  gameSettings: {
    open: "对局规则",
    title: "对局规则",
    recomputeHint: "更改会应用于整场对局：已计分的回合将按新规则重新计算。",
    close: "完成",
  },
  ...
  stats: {
    ...
    bestFinalScoreHint: "所有人结束对局时拿到过的最高总分。",
    biggestRound: "单回合最高",
    biggestRoundHint: "单个回合里赚到的最多分数。",
    bestExactBid: "叫牌最准",
    bestExactBidHint: (rounds) =>
      `精准叫牌率最高者，至少需打满 ${rounds} 个回合。`,
    zeroBidMaster: "零叫大师",
    zeroBidMasterHint: (zeroBids) =>
      `叫零并且一墩不吃的最佳战绩，至少需有 ${zeroBids} 次零叫。`,
    longestStreak: "最长连胜",
    longestStreakHint: "连续获胜场次最多的一段。",
    biggestComeback: "最大逆转",
    biggestComebackHint: "从半程排名到终局排名之间上升名次最多的一次。",
    biggestBonusHaul: "最丰厚战利品",
    biggestBonusHaulHint: "单局中靠特殊牌拿到的最多奖励分。",
    worstFinalScore: "最低终局得分",
    worstFinalScoreHint: "所有人结束对局时拿到过的最低总分。",
    worstRound: "最差回合",
    worstRoundHint: "单个回合里亏得最多的一次。",
    mostLastPlaces: "垫底次数最多",
    mostLastPlacesHint: "落在全桌最后一名的次数比谁都多。",
    boldestBidder: "叫牌最大胆",
    boldestBidderHint: "每手牌都叫下最大的份额：勇气还是鲁莽。",

    recordUnclaimed: "尚无人获得",
    unitPoints: "分",
    unitWins: () => "胜",
    unitPlaces: () => "名",
    unitGames: () => "局",
    roundMeta: (round, date) => `第 ${round} 回合 · ${date}`,
    sampleMeta: (successes, attempts) => `${attempts} 回合中 ${successes} 次`,
    comebackMeta: (fromRank, toRank, date) =>
      `第 ${fromRank} 名 → 第 ${toRank} 名 · ${date}`,
    lastPlaceMeta: (rate, games) => `${games} 局中占 ${rate}`,
    appetiteMeta: (averageBid, rounds) =>
      `每回合平均叫 ${averageBid} 墩 · 共 ${rounds} 回合`,
    ...
    bidAppetite: "叫牌胃口",
    averagePoints: "场均得分",
    pointsPerRound: "每回合得分",
    bestScore: "最高得分",
    worstScore: "最低得分",
    bestRoundScore: "最佳回合",
    worstRoundScore: "最差回合",
    bonusPoints: "奖励分",

    outOfGames: (count, games) => `${games} 局中 ${count} 次`,
    seatsCaption: (seats) => `共 ${seats} 名玩家`,
    perGame: "所有对局的平均值",
    rivalsBeatenCaption: "完赛时排在其前面的对手占比",
    perRound: "所有回合的平均值",
    fromSpecialCards: "来自特殊牌",
    bidCaption: (averageBid) => `每回合平均叫 ${averageBid} 墩`,
  ...
  scoreBreakdown: {
    ...
    items: {
      ...
      secondCaptured: "Skull King 或美人鱼俘获“副手”",
  ...
  rules: {
    ...
    scoring: [
      ...
      { title: "叫零", body: "赢 0 墩：本回合每发一张牌 +10。赢任何一墩：本回合每发一张牌 -10。" },
    ],
    rascal: [
      { title: "官方的另一种计分方式", body: "在创建对局时选择。每回合所有玩家的潜在得分相同，即每发一张牌 10 分，与叫牌数无关。最终拿到多少取决于叫牌的准确度。分数永远不会变成负数。" },
      ...
      { title: "可选规则：散弹或炮弹", body: "若在设置中启用，每人在叫牌后做出选择并同时亮出：张开手掌（散弹）按常规档位计分；握紧拳头（炮弹）在叫牌准确时每发一张牌得 15 分，否则一分不得，奖励分同样如此。" },
    ],
    bonusEntries: [
    ...
    expansion: [
      ...
      { title: "戴维·琼斯的箱子（每个利维坦 +20）", body: "与利维坦配合使用。它不能赢墩，并摧毁该墩所有利维坦；然后由剩余最强牌正常获胜。无论出牌顺序，打出箱子的玩家每摧毁一个利维坦得 20 分。" },
